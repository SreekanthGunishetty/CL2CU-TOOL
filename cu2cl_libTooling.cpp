...
//Global variables for kernel call conversion (try)
std::string a[100];
int count = 0;
std::string temp="";
std::string finalKerName="";
//std::string gridSize = "";
...
bool isInBannedInclude(SourceLocation loc, SourceManager * SM, LangOptions * LO) {
    ...
    if (filename(FileName).equals("CL/opencl.h") || SM->isInSystemHeader(loc) || SM->isInExternCSystemHeader(loc) || SM->isInSystemMacro(loc) || SM->isInSystemHeader(sloc) || SM->isInExternCSystemHeader(sloc) || SM->isInSystemMacro(sloc)) {
//	std::cout << "INSIDE 2ND IF OF ISINBANNEDINCLUDE" << std::endl;
...
private:
    ...
    bool RewriteHostExpr(Expr *e, std::string &newExpr) {
        ...
        if (CUDAKernelCallExpr *kce = dyn_cast<CUDAKernelCallExpr>(e)) {
            ...
            else if (ce->getDirectCallee()->getNameAsString().find("cl") == 0)
            {
	...
	//TRYING START
	else if (funcName == "clEnqueueWriteBuffer")
	{
		Expr* dst = cudaCall->getArg(1);
		Expr* src = cudaCall->getArg(5);
		Expr* count = cudaCall->getArg(4);
		std::string newDst, newSrc, newCount;
		RewriteHostExpr(dst, newDst);
		RewriteHostExpr(src, newSrc);
		RewriteHostExpr(count, newCount);
		newExpr = "cudaMemcpy(" + newDst + ", " + newSrc + ", " + newCount + "," + "cudaMemcpyHostToDevice)";
	}
	else if (funcName == "clEnqueueReadBuffer")
	{
		Expr* dst = cudaCall->getArg(1);
		Expr* src = cudaCall->getArg(5);
		Expr* count = cudaCall->getArg(4);
		...
		RewriteHostExpr(dst, newDst);
		RewriteHostExpr(src, newSrc);
		RewriteHostExpr(count, newCount); //TODO: support cudaMemcpyDefault
		newExpr = "cudaMemcpy(" + newSrc + ", " + newDst + ", " + newCount + "," + "cudaMemcpyDeviceToHost)"; 
//TODO support offsets (will need to grab pointer out of cudaMemcpy
	}        // call, then separate off the rest of the math as the offset)

	else if(funcName == "clSetKernelArg")
	{
		count++;
		Expr* index = cudaCall->getArg(1);
		Expr* arg = cudaCall->getArg(3);
		std::string newArg,newIndex;
		RewriteHostExpr(arg, newArg);
		RewriteHostExpr(index,newIndex);
		int idx = newIndex[0] - '0';
		size_t idx1 = newArg.find('&');
		a[idx] = newArg.substr(idx1+1);
		newExpr = "//";
	}

	else if(funcName == "clEnqueueNDRangeKernel")
	{
	//	std::cout << "INSIDENDRANGE" <<std::endl;
		Expr* kernel = cudaCall->getArg(1);
		Expr* GridSize = cudaCall->getArg(4);
		Expr* BlockSize = cudaCall->getArg(5);
		std::string newKernel,newGrid,newBlock;
		RewriteHostExpr(kernel, newKernel);
		RewriteHostExpr(GridSize, newGrid);
		RewriteHostExpr(BlockSize, newBlock);
		...
		int len = temp.length();
		temp = temp +  a[i];
		if(newGrid[0] == '&' && newBlock[0] == '&')
			newExpr = finalKerName + "<<<" + newGrid.substr(1) + "," + newBlock.substr(1) + ">>>(" + temp + ")";
	        else
			newExpr = finalKerName + "<<<" + newGrid + "," + newBlock + ">>>(" + temp + ")";
	}

	else if(funcName == "clCreateKernel")
	{     
		Expr* kernelName = cudaCall->getArg(1);
		std::string newKernelName;
		RewriteHostExpr(kernelName, newKernelName);
		int kerLen = newKernelName.length();
		finalKerName = newKernelName.substr(1,kerLen-2);
		newExpr = "clCreateKernel()";
	}

	else if(funcName == "clGetPlatformIDs" || funcName == "clGetDeviceIDs" || funcName == "clBuildProgram" || funcName == "clReleaseKernel" || funcName == "clReleaseCommandQueue" ||funcName == "clReleaseProgram" || funcName == "clReleaseContext")
	{
		newExpr = "//";
	}

	else if(funcName == "clFinish")
	{
		newExpr = "cudaDeviceSynchronize()";
	}
...
//                RewriteType(origTL, "cl_mem", HostReplace);
    ...
    void RewriteKernelFunction(FunctionDecl *kernelFunc) {
        ...
        if (CUDAConstantAttr *attr = kernelFunc->getAttr<CUDAConstantAttr>()) {
...
public:
	...
	HostIncludes += "#include <cuda.h>\n";
	HostIncludes += "#include <cuda_runtime.h>\n";
...
//        } 
    ...
    void RewriteInclude(SourceLocation HashLoc, const Token &IncludeTok,
        ...
        if (IsAngled) {
            if (!isInBannedInclude(HashLoc, SM, LO)){
//		 std::cout << "INSIDE IF AFTER ISANGLED" << std::endl;
		 generateReplacement(KernReplace, SM, HashLoc, getRangeSize(*SM, CharSourceRange::getTokenRange(SourceRange(HashLoc, EndLoc))), "");
            ...
            if (includedFile.equals("CL/opencl.h"))
	    {
//		std::cout << "INSIDE 2ND IF ISANGLED" <<std::endl;
        ...
        else if (includedFile.equals("CL/opencl.h")) {
            generateReplacement(HostReplace, SM, HashLoc, getRangeSize(*SM, CharSourceRange::getTokenRange(SourceRange(HashLoc, EndLoc))), "");
            generateReplacement(KernReplace, SM, HashLoc, getRangeSize(*SM, CharSourceRange::getTokenRange(SourceRange(HashLoc, EndLoc))), "");
        }
...
void replaceVarDecl(DeclaratorDecl *decl, SourceTuple * ST) {
    ...
    replace += "void" + pointers + " " + decl->getNameAsString();
    if (VarDecl *var = dyn_cast<VarDecl>(decl)) {
        ...
        generateReplacement(GlobalHostReplace, SM, tl.getBeginLoc(), getRangeSize(*SM, CharSourceRange::getTokenRange(tl.getLocalSourceRange())), "void");
    }
...
int main(int argc, const char ** argv) {
   ...
   // *cu2cl_header << "#endif\n";
    *cu2cl_header << "#include <cuda.h>\n";
    *cu2cl_header << "#include <cuda_runtime.h>\n";
