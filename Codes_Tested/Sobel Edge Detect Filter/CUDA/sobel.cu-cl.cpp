...
cl_program __cu2cl_Program_sobel_cu;
...
void __cu2cl_Cleanup_sobel_cu() {
  clReleaseKernel(__cu2cl_Kernel_sobelGpu);
  clReleaseProgram(__cu2cl_Program_sobel_cu);
}
void __cu2cl_Init_sobel_cu() {
#ifdef WITH_ALTERA
  progLen = __cu2cl_LoadProgramSource("sobel_cu_cl.aocx", &progSrc);
  __cu2cl_Program_sobel_cu = clCreateProgramWithBinary(__cu2cl_Context, 1, &__cu2cl_Device, &progLen, (const unsigned char **)&progSrc, NULL, NULL);
#else
  progLen = __cu2cl_LoadProgramSource("sobel.cu-cl.cl", &progSrc);
  __cu2cl_Program_sobel_cu = clCreateProgramWithSource(__cu2cl_Context, 1, &progSrc, &progLen, NULL);
#endif
  free((void *) progSrc);
  clBuildProgram(__cu2cl_Program_sobel_cu, 1, &__cu2cl_Device, "-I . ", NULL, NULL);
  __cu2cl_Kernel_sobelGpu = clCreateKernel(__cu2cl_Program_sobel_cu, "sobelGpu", NULL);
}
...
int main()
{
  ...
  *(void **)&d_out = clCreateBuffer(__cu2cl_Context, CL_MEM_READ_WRITE, (w * h), NULL, NULL);
  ...
  err = __cu2cl_Memset(d_out, 0, w * h);
  std::cout << getErrorString(err) << std::endl;
  ...
  cl_int cudaerror = clFinish(__cu2cl_CommandQueue);
  std::cout << getErrorString(cudaerror);
